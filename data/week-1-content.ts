...
Use the CERC framework to structure your argument. Explicitly verify all theorem conditions before drawing your conclusion.`,
    ...
    correctCERCResponse: {
      claim: "The Mean Value Theorem does not apply to this problem.",
      evidence: "The function f(x) = 1/x² is undefined at x = 0, which lies in the interval [-1, 1]. Evaluating the difference quotient: (f(1) - f(-1))/(1-(-1)) = (1 - 1)/2 = 0.",
      reasoning: "The Mean Value Theorem requires the function to be continuous on [a,b] and differentiable on (a,b). Since f is not continuous on [-1,1] (it has a discontinuity at x=0), the theorem's hypotheses are not satisfied.",
      conditions: "Checking continuity: f(x) = 1/x² is discontinuous at x = 0 because lim(x→0) f(x) = ∞ (the function is not defined at x=0). Therefore, the MVT cannot be applied.",
    },
    sentenceFrames: {
...
Use the CERC framework. Explicitly check whether the Intermediate Value Theorem applies.`,
    ...
    correctCERCResponse: {
      claim: "The Intermediate Value Theorem does not apply, so we cannot conclude that such a value c exists (though one does exist by direct observation, the IVT cannot be used to prove it).",
      evidence: "g(0) = 0² = 0, g(2) = 3, and 2 is between 0 and 3. However, g has a jump discontinuity at x = 1: lim(x→1⁻) g(x) = 1² = 1, but g(1) = 3.",
      reasoning: "The IVT requires continuity on [a,b]. Since g is not continuous at x = 1 (which is in [0,2]), the hypothesis is violated and the theorem cannot be applied.",
      conditions: "Checking continuity: g is continuous on [0,1) and on [1,2], but at x=1, lim(x→1⁻) g(x) = 1 ≠ 3 = g(1). Therefore g is not continuous on [0,2], and IVT does not apply.",
    },
    sentenceFrames: {
...
Apply the CERC framework and verify all conditions.`,
    ...
    correctCERCResponse: {
      claim: "The Mean Value Theorem does not apply to h(x) = |x| on [-2, 2].",
      evidence: "h(-2) = 2, h(2) = 2, so (h(2) - h(-2))/4 = 0. The function h is continuous everywhere, but at x = 0, the left derivative is -1 and the right derivative is +1.",
      reasoning: "MVT requires both continuity on [a,b] AND differentiability on (a,b). While h is continuous on [-2,2], it is not differentiable at x = 0 (which is in the open interval (-2,2)), so the theorem's hypotheses are not fully satisfied.",
      conditions: "Checking: (1) Continuity on [-2,2]: h is continuous everywhere. ✓ (2) Differentiability on (-2,2): h'(x) = -1 for x<0 and h'(x) = 1 for x>0, but h'(0) does not exist (sharp corner). ✗ Therefore MVT does not apply.",
    },
    sentenceFrames: {
...
**Task:** The researcher proposes using a two-sample t-test to determine if the mean post-test score is significantly higher than the mean pre-test score. Use the CERC framework to evaluate whether this inference procedure is appropriate.`,
    ...
    correctCERCResponse: {
      ...
      evidence: "The same 20 students were measured twice (pre-test and post-test). The data consist of paired observations, not two independent samples. The standard deviation of differences (8.2) has already been calculated, suggesting the data structure is paired.",
      reasoning: "The two-sample t-test requires that the two samples be independent (observations in one group should not influence observations in the other group). Since these are measurements from the same students before and after the intervention, the samples are dependent, not independent.",
      conditions: "Checking independence: The pre-test and post-test scores come from the same 20 students, so they are clearly dependent (a student's pre-test score is likely related to their post-test score). This violates the independence condition required for a two-sample t-test. The correct procedure would be a paired t-test.",
    },
    sentenceFrames: {
...
**Task:** Determine whether a one-sample t-test can be used to test the manager's claim at the α = 0.05 significance level. Use the CERC framework to justify your answer.`,
    ...
    correctCERCResponse: {
      claim: "A one-sample t-test should not be used for these data.",
      evidence: "The sample size is n = 8, which is small. The sample data appear skewed to the left, indicating the distribution is not approximately normal. The normality condition is not satisfied.",
      reasoning: "The one-sample t-test requires that either the population is approximately normal, or the sample size is large enough (n ≥ 30) for the Central Limit Theorem to ensure the sampling distribution is approximately normal. With n = 8 and skewed data, neither condition is met.",
      conditions: "Checking normality: The sample size n = 8 is small (< 30), so we cannot rely on CLT. The data are described as skewed, so the population is not approximately normal. Therefore, the normality condition required for the t-test is not satisfied.",
    },
    sentenceFrames: {
