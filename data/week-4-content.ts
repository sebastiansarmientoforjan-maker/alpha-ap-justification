...
- Volume of water: $V = \\pi r^2 h$ (cylinder)`,
    ...
    correctResponse: {
      phase1Solution: "The net rate is R_net(t) = R_in(t) - R_out(t) = (5 + 2sin(πt/6)) - (3 + 0.5t) = 2 + 2sin(πt/6) - 0.5t. To find maximum, take derivative: R_net'(t) = 2(π/6)cos(πt/6) - 0.5 = (π/3)cos(πt/6) - 0.5. Set equal to zero: (π/3)cos(πt/6) = 0.5, so cos(πt/6) = 1.5/π ≈ 0.477. Then πt/6 = arccos(0.477) ≈ 1.068 rad, giving t ≈ 2.04 minutes. To convert to height rate: dh/dt = R_net(t)/(πr²) = R_net(t)/(9π). Maximum height rate occurs at t ≈ 2.04 min.",
      phase2CERC: {
        claim: "By the Mean Value Theorem for Integrals, there exists a time c in (0,10) at which the instantaneous net flow rate equals the average net flow rate over the interval.",
        evidence: "Average rate = (1/10)∫₀¹⁰ R_net(t) dt. Compute: ∫₀¹⁰ [2 + 2sin(πt/6) - 0.5t] dt = [2t - 12(cos(πt/6))/(π) - 0.25t²]₀¹⁰ = 20 - 12cos(5π/3)/π - 25 - (-12/π) = -5 + 12(1-cos(5π/3))/π. Since cos(5π/3) = 0.5, this gives -5 + 12(0.5)/π ≈ -5 + 1.91 ≈ -3.09 m³. Average = -3.09/10 ≈ -0.309 m³/min. By MVT, ∃c such that R_net(c) = -0.309.",
        reasoning: "The MVT for integrals states that if f is continuous on [a,b], then there exists c in (a,b) such that f(c) equals the average value of f over [a,b]. Since R_net(t) is continuous (sum/difference of continuous trig and polynomial functions), MVT applies.",
        conditions: "Verifying MVT conditions: R_net(t) = 2 + 2sin(πt/6) - 0.5t is continuous on [0,10] because sine and polynomials are continuous everywhere. Therefore MVT for integrals applies, guaranteeing such a c exists."
      },
      phase3AdaptedConclusion: "With the clogged drain at t=7, only R_in flows for t∈[7,10]. Volume added = ∫₇¹⁰ (5+2sin(πt/6))dt ≈ 15.3 m³. Height increase = 15.3/(9π) ≈ 0.54 m. New height = 2 + (height change from 0 to 7) + 0.54. We need to calculate height at t=7 first. If h(7) was already near the limit, adding 0.54m could cause overflow. CONCLUSION: We must check if h(7) + 0.54 > 8. If so, the tank WILL overflow."
    },
    theoremInfo: {
...
- Significance level: α = 0.05`,
    ...
    correctResponse: {
      phase1Solution: "This is an EXPERIMENT because random assignment was used. The appropriate procedure is a TWO-SAMPLE Z-TEST FOR PROPORTIONS (not paired, since different patients). Conditions: (1) Random assignment ✓ (stated). (2) Independence: samples are independent (different patients), and 120 patients < 10% of all chronic pain patients ✓. (3) Success-failure: Drug: np₁=45≥10✓, n(1-p₁)=15≥10✓. Placebo: np₂=30≥10✓, n(1-p₂)=30≥10✓. All conditions satisfied.",
      phase2CERC: {
        claim: "We reject the null hypothesis. There is convincing statistical evidence at α=0.05 that the new drug CAUSES a higher proportion of patients to experience pain reduction compared to placebo.",
        evidence: "H₀: p_drug = p_placebo (no difference). Hₐ: p_drug > p_placebo. Sample proportions: p̂₁ = 45/60 = 0.75, p̂₂ = 30/60 = 0.50. Pooled proportion: p̂ = (45+30)/(60+60) = 75/120 = 0.625. Test statistic: z = (0.75-0.50)/√(0.625·0.375/60 + 0.625·0.375/60) = 0.25/√(0.00781) = 0.25/0.0884 ≈ 2.83. P-value = P(Z > 2.83) ≈ 0.0023. Since 0.0023 < 0.05, reject H₀.",
        reasoning: "A two-sample z-test compares proportions from two independent groups. The p-value represents the probability of observing a difference as extreme as ours (or more) if the null hypothesis were true. Since p-value < α, the data provide strong evidence against H₀. Because RANDOM ASSIGNMENT was used (experiment), we can conclude causation.",
        conditions: "Conditions verified in Phase 1: random assignment, independence, and success-failure all satisfied. Additionally, because this is an experiment (not observational), we can make causal claims about the drug's effectiveness."
      },
      phase3AdaptedConclusion: "The confounding variable (prior treatment) DOES affect our conclusion about causation. If 15/60 drug patients had prior similar treatment, they may have been predisposed to respond better regardless of the new drug. This introduces selection bias despite random assignment (the groups were not truly equivalent at baseline). ADAPTED CONCLUSION: While we still have evidence of a difference in proportions, we cannot confidently claim the drug CAUSED the improvement. The prior treatment is a confounding factor that may explain part or all of the observed effect. The study design is compromised."
    },
    theoremInfo: {
