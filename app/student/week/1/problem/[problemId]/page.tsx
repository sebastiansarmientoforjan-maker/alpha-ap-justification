...
export default function Week1ProblemSolver() {
  ...
  const calculateXP = () => {
    ...
    if (!sessionData.phases.selfCheck.solutionViewed) {
      bonuses.push({ name: "Solved Independently", amount: 10 });
      xp += 10;
    }
            ...
            {currentPhase === "justify" && (
              ...
              >
                ...
                <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-xl text-sm">
                  <p className="font-semibold text-blue-300 mb-2">💡 Sentence Frames (Week 1 Full Scaffolding):</p>
                  <div className="space-y-1 text-primary-200 text-xs">
                    <p><span className="font-bold text-accent-400">Claim:</span> {problem.sentenceFrames.claim}</p>
                    <p><span className="font-bold text-blue-400">Evidence:</span> {problem.sentenceFrames.evidence}</p>
                    <p><span className="font-bold text-purple-400">Reasoning:</span> {problem.sentenceFrames.reasoning}</p>
                    <p><span className="font-bold text-green-400">Conditions:</span> {problem.sentenceFrames.conditions}</p>
                  </div>
                </div>
